...
public class AnalysisTransformer_Poly_to_Mono extends SceneTransformer {
    ...
    protected void internalTransform(String arg0, Map<String, String> arg1) {
        ...
        for (SootMethod m : methods) {
            if(PA4.debugPolytoMono) System.out.println(m);  
        }
        //print virtual_edges, static_edges, poly_invokes, mono_invokes
        if (PA4.doPolytoMono == false) {
            System.out.println(PA4.cgtype +"  Stats starts %%%%%%%%%");
    ...
    protected static void collectStaticReport(SootMethod method, PointsToAnalysis pa) {
        ...
        for (Unit u : units) {
            ...
            if(PA4.debugPolytoMono)System.out.println("------------------Unit: " + u + " -------------------");
            
            //Printing the edges of the call graph
            if(PA4.debugPolytoMono) System.out.println();
            int edge_count_out = 0;
            while (edges.hasNext()) {
                Edge edge = edges.next();
                if(PA4.debugPolytoMono) System.out.println("Printing edge:");
                if(PA4.debugPolytoMono) System.out.println(edge);
                SootMethod targetMethod = edge.tgt();
                if(PA4.debugPolytoMono) System.out.println("Printing method within edge:");
                if(PA4.debugPolytoMono) System.out.println(targetMethod);
                edge_count_out+=1;
                if(PA4.debugPolytoMono) System.out.println();
            }
            if(PA4.debugPolytoMono) System.out.println();
    ...
    protected static void processCFG(SootMethod method, PointsToAnalysis pa) {
        ...
        for (Unit u : units) {
            ...
            if(PA4.debugPolytoMono) System.out.println("Printing points to set for receiver if virtual invoke is present: ");
            if(u instanceof JInvokeStmt){
                JInvokeStmt invokeStmt = (JInvokeStmt) u;
                if(invokeStmt.getInvokeExpr() instanceof VirtualInvokeExpr){
                    VirtualInvokeExpr v_invoke= (VirtualInvokeExpr) invokeStmt.getInvokeExpr();
                    if(v_invoke.getMethod() != null && v_invoke.getMethod().isJavaLibraryMethod() == false){
                        ...
                        if(PA4.debugPolytoMono) System.out.println("Direct Invoke Points to set for receiver: " + receiver + " is: ");
                        for(Type t : pts.possibleTypes()){
                            if(PA4.debugPolytoMono) System.out.println(t);
                        }

                        if(pts.possibleTypes().size() == 1){
                            // if(PA4.debugPolytoMono) System.out.println("Printing polytomonopermision   "+PA4.doPolytoMono);
                    ...
                    if(PA4.debugPolytoMono) System.out.println(" XXXXX Direct Invoke but NO virtual invoke present in the unit");
                }
            }
            else if(u instanceof JAssignStmt){
                JAssignStmt assignStmt = (JAssignStmt) u;
                if(assignStmt.containsInvokeExpr() && assignStmt.getInvokeExpr() instanceof VirtualInvokeExpr){
                    ...
                    if(v_invoke.getMethod() != null && v_invoke.getMethod().isJavaLibraryMethod() == false){
                        Local receiver = (Local)v_invoke.getBase();
                        PointsToSet pts = pa.reachingObjects(receiver);
                        if(PA4.debugPolytoMono) System.out.println("AssignStmt Invoke Points to set for receiver: " + receiver + " is: ");
                        for(Type t : pts.possibleTypes()){
                            if(PA4.debugPolytoMono) System.out.println(t);
                        }
                    ...
                    if(PA4.debugPolytoMono) System.out.println(" XXXXX AssignStmt Invoke but NO virtual invoke present in the unit");
                }
            }
